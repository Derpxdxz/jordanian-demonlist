...
export default {
        ...
        <main v-else class="page-roulette">
            <div class="sidebar">
                <p class="type-label-md" style="color: #aaa">
                    {{ store.t('Shameless copy of the Extreme Demon Roulette by') }} <a href="https://matcool.github.io/extreme-demon-roulette/" target="_blank">matcool</a>.
                </p>
                <form class="options">
                    <div class="check">
                        <input type="checkbox" id="main" value="Main List" v-model="useMainList">
                        <label for="main">{{ store.t('Main List') }}</label>
                    </div>
                    <div class="check">
                        <input type="checkbox" id="extended" value="Extended List" v-model="useExtendedList">
                        <label for="extended">{{ store.t('Extended List') }}</label>
                    </div>
                    <Btn @click.native.prevent="onStart">{{ levels.length === 0 ? store.t('Start') : store.t('Restart') }}</Btn>
                </form>
                <p class="type-label-md" style="color: #aaa">
                    {{ store.t('The roulette saves automatically.') }}
                </p>
                <form class="save">
                    <p>{{ store.t('Manual Load/Save') }}</p>
                    <div class="btns">
                        <Btn @click.native.prevent="onImport">{{ store.t('Import') }}</Btn>
                        <Btn :disabled="!isActive" @click.native.prevent="onExport">{{ store.t('Export') }}</Btn>
                    </div>
                </form>
            </div>
                    ...
                    <template v-if="levels.length > 0">
                        ...
                        <div class="level" v-if="!hasCompleted">
                            ...
                            <form class="actions" v-if="!givenUp">
                                ...
                                <Btn @click.native.prevent="onDone">{{ store.t('Done') }}</Btn>
                                <Btn @click.native.prevent="onGiveUp" style="background-color: #e91e63;">{{ store.t('Give Up') }}</Btn>
                            </form>
                        </div>
                        <!-- Results -->
                        <div v-if="givenUp || hasCompleted" class="results">
                            <h1>{{ store.t('Results') }}</h1>
                            <p>{{ store.t('Number of levels:') }} {{ progression.length }}</p>
                            <p>{{ store.t('Highest percent:') }} {{ currentPercentage }}%</p>
                            <Btn v-if="currentPercentage < 99 && !hasCompleted" @click.native.prevent="showRemaining = true">{{ store.t('Show remaining levels') }}</Btn>
                        </div>
                        <!-- Remaining Levels -->
    ...
    computed: {
        ...
        placeholder() {
            return this.store.t('At least') + ' ' + (this.currentPercentage + 1) + '%';
        },
        hasCompleted() {
            return (
    ...
    methods: {
        ...
        async onStart() {
            if (this.isActive) {
                this.showToast(this.store.t('Give up before starting a new roulette.'));
                return;
            }
            ...
            if (fullList.filter(([_, err]) => err).length > 0) {
                this.loading = false;
                this.showToast(
                    this.store.t('List is currently broken. Wait until it\'s fixed to start a roulette.'),
                );
                return;
            }
